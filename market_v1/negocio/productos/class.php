    ...
    class productos extends conexion{
        ...
        public function mostrarProductos($pag) {
            ...
            while($this->registros = $this->consulta->fetch_array()){
                ?>
                    <tr>
                        <td><img src="fotos/<?php echo $this->registros['codigo']; ?>"
                                 style="width: 100px; height: 100px; margin: 0"></td>
                  ...
                  <td colspan="16" class="text-center">
                    ...
                    <ul class="pagination">
                        ...
                        <li class="page-item"><a class="page-link" 
                            href="index.php?pagina=<?php echo $_GET['pagina']-1 ?>">Anterior</a></li>
                <?php
                        ...
                        <select class="form-control" name="idrubro" required="">
                            ...
                            <?php 
                                 ...
                                 while($this->registros2 = $this->consulta2->fetch_array()){
                                     ?>
                            <option value="<?php echo $this->registros2['idrubro'];?>" <?php if($this->registros2['idrubro'] == $this->registros['idrubro']){ echo"selected='selected'";}?>><?php echo $this->registros2['nombrerubros'];?></option>
                                     <?php
                                 }
                            ?>
                        </select>
        ...
        switch ($this->tipo){
            case 'codigo': $this->consulta=$this->con->query("SELECT producto.*, rubros.* FROM producto INNER JOIN rubros ON producto.idrubro = rubros.idrubro where producto.codigo = '$this->buscar'");
                break;
            case 'nombre': $this->consulta=$this->con->query("SELECT producto.*, rubros.* FROM producto INNER JOIN rubros ON producto.idrubro = rubros.idrubro where producto.nombreproducto LIKE '%$this->buscar%'");
                break;
            case 'descripcion': $this->consulta=$this->con->query("SELECT producto.*, rubros.* FROM producto INNER JOIN rubros ON producto.idrubro = rubros.idrubro where producto.descripcion LIKE '%$this->buscar%'");
        }
        
        while($this->recorrido = $this->consulta->fetch_array()){
            ?>
            <tr>
                <td><img src="fotos/<?php echo $this->recorrido['codigo']; ?>"
                         style="width: 100px; height: 100px; margin: 0"></td>
